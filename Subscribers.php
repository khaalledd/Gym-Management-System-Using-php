<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(empty($id) ||empty($name) ||empty($mobile) || empty($e_mail) || empty($price)) {
            ...
            if(strlen($name) >= 50 || !preg_match("/^[a-zA-Z-'\s]+$/", $name)) {
                ...
                include('connect.php');
                $sql = "INSERT INTO members (id, name, mobile, e_mail, gender, price, date) VALUES (:id, :name, :mobile, :e_mail, :gender, :price, :date)";
                $stmt = $conn->prepare($sql);
                $stmt->execute(['id' => $id,'name' => $name,'mobile' => $mobile, 'e_mail' => $e_mail, 'gender' => $gender,'price' => $price,'date' => $date]);
                $err = "successfully Submited";
                $id= "";
                $name= "";
                $mobile= "";
                $e_mail= "";
...
<body>
    ...
    <div class="container">
        ...
        <form action="" method="post">
            <table class="box">
                <tr>
                    <td><label for="">Member_ID: </label></td>
                ...
                <tr>
                    <td><input class="ha" type="submit" value="Send" name="send"></td>
